...
def tracking(case,base_date,lx,ly,led,grid_bnds,dx,constant_feature,set_width,set_height,gauss_smooth,offset,t_frames,frame_dt,skip_times=0,save_fig=True):
    ...
    for m,(nt,i) in enumerate(zip(t_frames[:],np.arange(t_frames.shape[0])[:])):
        ...
        for region in regionprops(label_image):
            labels_all = np.unique(label_image[label_image!=0])
            ...
            if labels_all.shape[0]>1:
                second_object_size = np.where(label_image==labels_all.max())[0].shape[0]
            else:
                second_object_size = 0
            #Get all centroids to ensure tracked object is always within analysis domain area
            all_centroid_x0 = gmx[int(region.centroid[0]),int(region.centroid[1])]
            all_centroid_y0 = gmy[int(region.centroid[0]),int(region.centroid[1])]
            ...
            if (region.area == object_size):
                get_region = region
                get_xcent = all_centroid_x0*1e-3
                get_ycent = all_centroid_y0*1e-3
